...
const copy = {
  en: {
    explore: "Explore", trip: "My Trip", rewards: "GemPoints", about: "About", account: "Profile",
  ...
  it: {
    explore: "Esplora", trip: "Il mio viaggio", rewards: "GemPoints", about: "Informazioni", account: "Profilo",
  ...
  de: {
    explore: "Entdecken", trip: "Meine Reise", rewards: "GemPoints", about: "Über GemGo", account: "Profil",
  ...
  fr: {
    explore: "Explorer", trip: "Mon voyage", rewards: "GemPoints", about: "À propos", account: "Profil",
  ...
  sl: {
    explore: "Razišči", trip: "Moje potovanje", rewards: "GemPoints", about: "O projektu", account: "Profil",
...
const tr = (locale: Locale, english: string, italian: string) => locale === "it" ? italian : english;
...
const badgeValuesFor = (events: GemPointEvent[], trips: SavedTrip[]) => {
  ...
  return [
    ...
    visits.filter((event) => ["walking", "bicycle", "public"].includes(event.metadata?.transport ?? "")).length,
...
export default function IntegratedAppShell() {
  ...
  const routeTimes = useRoadTimes(origin, stage === "brief" ? [] : roadCandidates, preferences.transport);
  ...
  const selectedRoute = useSelectedRoute(origin, stage === "brief" ? null : selected ?? null, preferences.transport);
  const activeRoute = useSelectedRoute(origin, section === "trip" ? activeExperience : null, activeTrip?.preferences.transport ?? preferences.transport);
  ...
  const completeVerification = (status: "demo" | "verified") => {
    ...
    const unlockedIndex = goals.findIndex((goal, index) => beforeBadges[index] < goal && afterBadges[index] >= goal);
    if (unlockedIndex >= 0) {
      const badgeTitle = gemPointsCopy[locale].badgeNames[unlockedIndex];
      const history = JSON.parse(window.localStorage.getItem(BADGE_HISTORY_KEY) ?? "[]") as Array<{ id: string; title: string; createdAt: string }>;
      window.localStorage.setItem(BADGE_HISTORY_KEY, JSON.stringify([{ id: `badge-${Date.now()}`, title: badgeTitle, createdAt: new Date().toISOString() }, ...history]));
    ...
    <main className="product-app integrated-app">
      <header className="app-header">
        ...
        <nav className="desktop-nav" aria-label={t.explore}>
          {navItems.map((item) => item.id === "about" ? <Link key={item.id} href="/about">{item.label}</Link> : <button type="button" key={item.id} className={section === item.id ? "is-active" : ""} onClick={() => chooseSection(item.id)}>{item.label}</button>)}
        </nav>
        <div className="header-actions">
          ...
          <Link href="/profile" className="icon-text-button profile-page-link"><UserRound size={18} /><span>{t.account}</span></Link>
        ...
        {mobileMenuOpen && <nav className="mobile-menu">{navItems.map((item) => item.id === "about" ? <Link key={item.id} href="/about">{item.label}<ChevronRight size={17} /></Link> : <button type="button" key={item.id} onClick={() => chooseSection(item.id)}>{item.label}<ChevronRight size={17} /></button>)}<Link href="/profile">{t.account}<ChevronRight size={17} /></Link></nav>}
      </header>
      ...
      {section === "explore" && (
        <section className="app-content explore-page">
            ...
            <>
              ...
              <div className="explore-layout">
                <form className="planner-panel" onSubmit={(event) => { event.preventDefault(); search(); }}>
                  ...
                  <div className="form-section"><div className="form-section-title"><LocateFixed size={18} /><div><strong>{t.origin}</strong><small>{originStatus === "ready" ? origin?.label : originStatus === "loading" ? u.resolving : originStatus === "not-found" ? u.notFound : u.typePlace}</small></div></div><div className="field-grid field-grid-location"><label><span>{t.origin}</span><input autoComplete="off" value={preferences.origin} onChange={(event) => updatePreference("origin", event.target.value)} /></label><label><span>{t.travel}</span><div className="range-value">{formatDuration(preferences.maxTravelMinutes)}</div><input type="range" min="15" max="720" step="15" value={preferences.maxTravelMinutes} onChange={(event) => updatePreference("maxTravelMinutes", Number(event.target.value))} /></label></div></div>
                ...
                <aside className="explore-aside"><div className="catalogue-card"><span className="eyebrow"><Globe2 size={14} /> {u.catalogue}</span><strong>{totalCatalogueEntries}</strong><span>{u.catalogueBody}</span><div>{Object.entries(catalogueSummary).map(([region, count]) => <small key={region}>{region}: {count}</small>)}</div></div><ExperienceMap locale={locale} experiences={ranked.map((item) => item.experience)} origin={origin} selectedId={selectedId} routeCoordinates={selectedRoute} onSelect={(experience) => setSelectedId(experience.id)} /><div className="method-card"><h3>{u.quality}</h3><p>{u.qualityBody}</p></div></aside>
              </div>
            </>
          )}
          ...
          {stage === "results" && (
            <>
              <div className="results-header"><button type="button" className="back-button" onClick={() => setStage("brief")}><ArrowLeft size={17} /> {t.adjust}</button><div><span className="eyebrow"><Sparkles size={15} /> {tr(locale,"Ranked from","Classificate tra")} {totalCatalogueEntries} {tr(locale,"entries","destinazioni")}</span><h1>{t.results}</h1><p>{tr(locale,"Starting from","Partenza da")} <strong>{origin?.label ?? preferences.origin}</strong>, {tr(locale,"within","entro")} <strong>{formatDuration(preferences.maxTravelMinutes)}</strong>.</p></div><div className="results-summary"><strong>3</strong><span>{tr(locale,"distinct recommendation roles","alternative con ruoli distinti")}</span><small>{tr(locale,"Live route times where available","Tempi di percorso live quando disponibili")}</small></div></div>
              <div className="result-layout"><div className="results-map-panel"><ExperienceMap locale={locale} experiences={ranked.map((item) => item.experience)} origin={origin} selectedId={selectedId} routeCoordinates={selectedRoute} onSelect={(experience) => setSelectedId(experience.id)} /><div className="comparison-proof"><div><span>{tr(locale,"Original plan","Piano originale")}</span><strong>{tr(locale,"Popular destination","Destinazione popolare")}</strong><small>{tr(locale,"Higher expected pressure","Pressione prevista più alta")}</small></div><ArrowRight size={22} /><div><span>{tr(locale,"GemGo alternative","Alternativa GemGo")}</span><strong>{ranked[0]?.experience.name}</strong><small>{tr(locale,"Compatible and more transparent","Compatibile e più trasparente")}</small></div></div></div><div className="result-cards">{ranked.map((item) => <IntegratedResultCard locale={locale} key={item.experience.id} item={item} saved={savedTrips.some((trip) => trip.trip.experienceId === item.experience.id)} onOpen={() => { setSelectedId(item.experience.id); setStage("experience"); window.scrollTo({ top: 0, behavior: "smooth" }); }} onSave={() => saveIdea(item.experience)} />)}{showMore && moreResults.map((experience) => <CompactResult key={experience.id} experience={experience} onOpen={() => { setSelectedId(experience.id); setStage("experience"); }} />)}<button type="button" className="show-more-button" onClick={() => setShowMore((value) => !value)}>{showMore ? tr(locale,"Show only the top three","Mostra solo le prime tre") : tr(locale,"Show more compatible results","Mostra altre alternative compatibili")}<ChevronRight size={17} /></button></div></div>
            </>
          )}

          {stage === "experience" && selected && <ExperienceDetail locale={locale} experience={selected} ranked={ranked.find((item) => item.experience.id === selected.id)} origin={origin} route={selectedRoute} transport={preferences.transport} onBack={() => setStage("results")} onSave={() => saveIdea(selected)} onAdd={() => saveIdea(selected, true)} />}
        </section>
      )}

      {section === "trip" && (
        <section className="app-content trip-page integrated-trip-page">
          <div className="page-heading trip-heading"><div><span className="eyebrow"><CalendarDays size={15} /> {t.trip}</span><h1>{activeExperience ? activeTrip?.name : t.noTrip}</h1><p>{tr(locale,"Saved ideas and active operational plans now live in one place.","Idee salvate e piani attivi sono raccolti in un unico posto.")}</p></div><div className="segmented-control"><button type="button" className={tripMode === "active" ? "is-active" : ""} onClick={() => setTripMode("active")}>{t.active}</button><button type="button" className={tripMode === "saved" ? "is-active" : ""} onClick={() => setTripMode("saved")}>{t.saved} ({savedTrips.length})</button></div></div>
          {tripMode === "active" && (!activeTrip || !activeExperience ? <div className="empty-state"><CalendarDays size={36} /><h2>{t.noTrip}</h2><button type="button" className="button button-primary" onClick={() => chooseSection("explore")}>{t.find}<ArrowRight size={17} /></button></div> : <div className="trip-layout"><div className="trip-main-card"><DestinationPhoto name={activeExperience.name} region={activeExperience.region} className="trip-real-photo" /><div className="trip-title-block"><span className={`crowd-chip crowd-${activeExperience.crowd}`}>{activeExperience.crowd} {tr(locale,"crowd","affollamento")}</span><h2>{activeExperience.promise}</h2><p>{activeExperience.name} · {tr(locale,"departure","partenza")} {activeTrip.trip.plannedDeparture}</p></div><ExperienceMap experiences={[activeExperience]} origin={origin} selectedId={activeExperience.id} routeCoordinates={activeRoute} /><div className="trip-timeline">{activeExperience.itinerary.map((item, index) => <div className="timeline-item" key={`${item.time}-${item.label}`}><div className="timeline-marker">{index + 1}</div><time>{item.time}</time><span>{item.label}</span></div>)}</div><div className="trip-action-row"><button type="button" className="button button-primary" onClick={() => setVerificationOpen(true)} disabled={activeTrip.trip.verified}>{activeTrip.trip.verified ? tr(locale,"Visit verified","Visita verificata") : t.verify}<BadgeCheck size={17} /></button><a className="button button-secondary" href={`https://www.openstreetmap.org/directions?to=${activeExperience.latitude},${activeExperience.longitude}`} target="_blank" rel="noreferrer"><Navigation size={17} /> {tr(locale,"Open navigation","Apri navigazione")}</a></div></div><aside className="trip-side"><div className="operational-card"><h3>{tr(locale,"Operational information","Informazioni operative")}</h3>{activeExperience.mobility.map((item) => <p key={item}><CheckCircle2 size={16} />{item}</p>)}<p><Users size={16} /> {tr(locale,"Best lower-pressure window:","Fascia consigliata a minore affollamento:")} {activeExperience.crowdWindow}</p></div><div className="offline-card"><Download size={22} /><div><h3>{tr(locale,"Offline essentials","Dati essenziali offline")}</h3><p>{tr(locale,"Cache the application shell and preserve this trip on the device.","Salva l’app e questo viaggio sul dispositivo.")}</p></div><button type="button" className="button button-secondary" onClick={saveOffline}>{activeTrip.offlineSaved ? tr(locale,"Saved offline","Salvato offline") : t.offline}</button></div><button type="button" className="condition-change-card" onClick={() => setGemDropOpen(true)}><Sparkles size={22} /><span><strong>{tr(locale,"Conditions changed","Condizioni cambiate")}</strong><small>{tr(locale,"Compare a contextual quieter alternative.","Confronta un’alternativa contestuale più tranquilla.")}</small></span><ChevronRight size={18} /></button></aside></div>)}
          {tripMode === "saved" && <div className="saved-trip-grid">{savedTrips.length === 0 ? <div className="empty-state"><Save size={34} /><h2>{tr(locale,"No saved trips","Nessun viaggio salvato")}</h2></div> : savedTrips.map((trip) => { const experience = allExperiences.find((item) => item.id === trip.trip.experienceId); return <article className="saved-trip-card" key={trip.id}><div><span>{experience?.region ?? "Alps"}</span><h3>{trip.name}</h3><p>{experience?.promise}</p><small>{tr(locale,"Updated","Aggiornato")} {new Date(trip.updatedAt).toLocaleDateString(locale)}</small></div><div className="saved-trip-actions"><button type="button" onClick={() => { setActiveTrip(trip); setTripMode("active"); }}><ArrowRight size={16} /> {tr(locale,"Open","Apri")}</button><button type="button" onClick={() => renameTrip(trip)}><Pencil size={16} /> {tr(locale,"Rename","Rinomina")}</button><button type="button" onClick={() => duplicateTrip(trip)}><Copy size={16} /> {tr(locale,"Duplicate","Duplica")}</button><button type="button" onClick={() => deleteTrip(trip)}><Trash2 size={16} /> {tr(locale,"Delete","Elimina")}</button></div></article>; })}</div>}
        </section>
      )}
      ...
      {gemDropOpen && activeExperience && gemDropAlternative && <GemDropModal locale={locale} original={activeExperience} alternative={gemDropAlternative} onClose={() => setGemDropOpen(false)} onSwitch={switchGemDrop} />}
      {verificationOpen && activeExperience && <VerificationModal locale={locale} experience={activeExperience} message={verificationMessage} qrCode={qrCode} onQrCode={setQrCode} onGps={verifyGps} onQr={verifyQr} onDemo={() => completeVerification("demo")} onClose={() => setVerificationOpen(false)} />}
      {toast && <div className="action-toast"><CheckCircle2 size={18} />{toast}</div>}
    </main>
  );
}

function CompactResult({ experience, onOpen }: { locale: Locale; experience: Experience; onOpen: () => void }) {
...
function ExperienceDetail({ locale, experience, ranked, origin, route, transport, onBack, onSave, onAdd }: { locale: Locale; experience: Experience; ranked?: RankedExperience; origin: { label: string; lat: number; lng: number } | null; route: Array<[number, number]>; transport: TransportMode; onBack: () => void; onSave: () => void; onAdd: () => void }) {
  const travel = ranked?.travelMinutes ?? experience.travel[transport];
  return <div className="experience-detail"><button type="button" className="back-button" onClick={onBack}><ArrowLeft size={17} />{tr(locale,"Back to alternatives","Torna alle alternative")}</button><div className="integrated-detail-hero"><DestinationPhoto name={experience.name} region={experience.region} /><div className="detail-hero-copy"><div><span>{experience.validation}</span><span className={`crowd-chip crowd-${experience.crowd}`}>{experience.crowd} {tr(locale,"crowd","affollamento")}</span></div><p>{experience.region} · {experience.country}</p><h1>{experience.promise}</h1><strong>{experience.name}</strong></div></div><div className="detail-metric-strip"><span><Navigation size={18} /><strong>{travel ? formatDuration(travel) : tr(locale,"Unavailable","Non disponibile")}</strong><small>{tr(locale,"travel time","tempo di viaggio")}</small></span><span><Clock3 size={18} /><strong>{formatDuration(experience.durationMinutes)}</strong><small>{tr(locale,"duration","durata")}</small></span><span><Users size={18} /><strong>{experience.crowdWindow}</strong><small>{tr(locale,"best window","fascia migliore")}</small></span><span><Coins size={18} /><strong>+{experience.points}</strong><small>GemPoints</small></span></div><div className="detail-grid"><div className="detail-main"><section className="content-card fit-card"><span className="eyebrow"><Sparkles size={14} />{tr(locale,"Why this fits you","Perché è adatta a te")}</span><h2>{tr(locale,"A recommendation derived from your real constraints","Una raccomandazione basata sui tuoi vincoli reali")}</h2><p>{ranked?.reasons.join(". ") ?? experience.summary}</p></section><section className="content-card"><h2>{tr(locale,"Route and mobility","Percorso e mobilità")}</h2><ExperienceMap experiences={[experience]} origin={origin} selectedId={experience.id} routeCoordinates={route} />{experience.mobility.map((item) => <p key={item}><CheckCircle2 size={16} />{item}</p>)}</section><section className="content-card"><h2>{tr(locale,"What you will do","Cosa farai")}</h2><div className="mini-itinerary">{experience.itinerary.map((item) => <div key={`${item.time}-${item.label}`}><time>{item.time}</time><span>{item.label}</span></div>)}</div></section><section className="content-card comparison-card"><h2>{tr(locale,"Honest comparison","Confronto trasparente")}</h2><div className="comparison-columns"><div><small>{tr(locale,"Original plan","Piano originale")}</small><strong>{experience.comparison.original}</strong><span>{experience.comparison.reachDifference}</span></div><ArrowRight size={22} /><div><small>{tr(locale,"GemGo alternative","Alternativa GemGo")}</small><strong>{experience.name}</strong>{experience.comparison.advantages.map((item) => <span key={item}><Check size={14} />{item}</span>)}</div></div><div className="tradeoff-box"><AlertTriangle size={18} /><div><strong>{tr(locale,"Trade-offs","Compromessi")}</strong>{experience.tradeoffs.map((item) => <span key={item}>{item}</span>)}</div></div></section></div><aside className="detail-side"><section className="content-card local-benefit-card"><HeartHandshake size={24} /><h3>{tr(locale,"Local benefit","Beneficio locale")}</h3><p>{experience.localBenefit}</p>{experience.partner && <span>{experience.partner}</span>}</section><section className="content-card safety-card"><h3>{tr(locale,"Safety and limits","Sicurezza e limiti")}</h3>{experience.safety.map((item) => <p key={item}><ShieldCheck size={16} />{item}</p>)}</section><div className="detail-cta-card"><button type="button" className="button button-primary button-full" onClick={onAdd}>{tr(locale,"Add to My Trip","Aggiungi al mio viaggio")}<ArrowRight size={17} /></button><button type="button" className="button button-secondary button-full" onClick={onSave}>{tr(locale,"Save for later","Salva per dopo")}</button><button type="button" className="button button-ghost button-full" onClick={onBack}>{tr(locale,"Show another alternative","Mostra un’altra alternativa")}</button></div></aside></div></div>;
}

function GemDropModal({ locale, original, alternative, onClose, onSwitch }: { locale: Locale; original: Experience; alternative: Experience; onClose: () => void; onSwitch: () => void }) {
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="gemdrop-panel"><button type="button" className="modal-close" onClick={onClose}><X size={20} /></button><span className="eyebrow"><Sparkles size={15} />{tr(locale,"Contextual GemDrop","GemDrop contestuale")}</span><h2>{tr(locale,"This area is becoming crowded","Questa area si sta affollando")}</h2><p>{tr(locale,"A comparable alternative is available. You remain in control.","È disponibile un’alternativa comparabile. La scelta resta tua.")}</p><div className="gemdrop-comparison"><div className="gemdrop-option original-option"><small>{tr(locale,"Original plan","Piano originale")}</small><h3>{original.name}</h3><p><Users size={16} />{tr(locale,"Crowd rising","Affollamento in aumento")}</p><p><Coins size={16} />{tr(locale,"Standard reward","Ricompensa standard")}</p></div><ArrowRight size={24} /><div className="gemdrop-option alternative-option"><small>{tr(locale,"Alternative","Alternativa")}</small><h3>{alternative.name}</h3><p><Users size={16} />{alternative.crowd} crowd</p><p><Coins size={16} />+20 GemPoints bonus</p></div></div><div className="gemdrop-actions"><button type="button" className="button button-primary" onClick={onSwitch}>{tr(locale,"Switch my trip","Cambia il mio viaggio")}<ArrowRight size={17} /></button><button type="button" className="button button-secondary" onClick={onClose}>{tr(locale,"Keep original plan","Mantieni il piano originale")}</button></div></div></div>;
}

function VerificationModal({ locale, experience, message, qrCode, onQrCode, onGps, onQr, onDemo, onClose }: { locale: Locale; experience: Experience; message: string; qrCode: string; onQrCode: (value: string) => void; onGps: () => void; onQr: () => void; onDemo: () => void; onClose: () => void }) {
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="verification-card verification-modal"><button type="button" className="modal-close" onClick={onClose}><X size={20} /></button><BadgeCheck size={36} /><h2>{tr(locale,"Verify your visit","Verifica la visita")}</h2><p>{tr(locale,"Verification awards points and measures whether recommendations redistribute flows without retaining a detailed movement history.","La verifica assegna punti e misura la redistribuzione dei flussi senza conservare uno storico dettagliato degli spostamenti.")}</p><button type="button" className="verification-method" onClick={onGps}><LocateFixed size={24} /><span><strong>{tr(locale,"Verify current location","Verifica la posizione attuale")}</strong><small>{tr(locale,"Checks whether you are within 2 km of","Controlla se ti trovi entro 2 km da")} {experience.name}.</small></span></button><div className="qr-verification"><label><span>{tr(locale,"Partner QR code","Codice QR del partner")}</span><input value={qrCode} onChange={(event) => onQrCode(event.target.value)} placeholder="GEMGO-…" /></label><button type="button" className="button button-secondary" onClick={onQr}><QrCode size={17} />{tr(locale,"Verify code","Verifica codice")}</button></div><button type="button" className="button button-ghost" onClick={onDemo}>{tr(locale,"Use clearly labelled demo verification","Usa la verifica demo chiaramente indicata")}</button>{message && <p className="verification-message">{message}</p>}</div></div>;
}
