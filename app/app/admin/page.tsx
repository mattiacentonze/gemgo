...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: queueError } = await supabase
      ...
      .select("id,author_id,name,description,region,category,map_url,status,created_at,contribution_media(object_path)")
      .eq("status", "pending")
  ...
  return (
    <main className="profile-page-v2 admin-page">
      ...
      <section className="profile-badges-v2"><div className="section-intro"><h2>{t.queue}</h2></div>{suggestions.length === 0 ? <p>{t.empty}</p> : <div className="admin-contribution-grid">{suggestions.map((item) => <article className="profile-auth-card" key={item.id}><span className="eyebrow">{t.pending} · {item.region} · {item.category}</span><h3>{item.name}</h3><p>{item.description}</p><small>{new Date(item.created_at).toLocaleString(locale)}</small><p>{item.map_url && <a href={item.map_url} target="_blank" rel="noreferrer noopener">{t.source}</a>}{photoUrls[item.id] && <> · <a href={photoUrls[item.id]} target="_blank" rel="noreferrer noopener">{t.photo}</a></>}</p>{canDecide && auth.user?.id !== item.author_id && <><label><span>{t.reason}</span><textarea value={notes[item.id] ?? ""} onChange={(event) => setNotes({ ...notes, [item.id]: event.target.value })} /></label><div className="profile-data-actions"><button disabled={busy === item.id} onClick={() => void decide(item.id, "approved")}><CheckCircle2 />{t.approve}</button><button disabled={busy === item.id || !notes[item.id]?.trim()} onClick={() => void decide(item.id, "rejected")}><XCircle />{t.reject}</button></div></>}{auth.user?.id === item.author_id && <p>{t.self}</p>}</article>)}</div>}</section>
